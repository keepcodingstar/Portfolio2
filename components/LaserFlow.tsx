...
/**
 * LaserFlow — a volumetric WebGL laser beam (React Bits). A full-screen raw
 * shader draws a flowing beam with wisps and fog; it adapts its pixel ratio to
 * hold framerate, pauses when off-screen or the tab is hidden, and tilts the
 * fog toward the pointer. Non-interactive by default (the container is
 * pointer-events: none). Drop it into a `position: relative` parent and size
 * that parent — the canvas fills 100% of it.
 */
...
export interface LaserFlowProps {
  className?: string;
  style?: CSSProperties;
  /** Density of micro-streak wisps. */
  wispDensity?: number;
  /** Device pixel ratio override (defaults to window.devicePixelRatio, capped at 2). */
  dpr?: number;
  /** Pointer smoothing time, in seconds. */
  mouseSmoothTime?: number;
  /** How much mouse x tilts the fog volume. */
